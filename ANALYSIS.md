...
| **Nop.Web.Framework** | Nop.Core, Nop.Data, Nop.Services | `Nop.Web.Framework.csproj:29-31` |
...
- **The engine**: `NopEngine.cs` (231 lines, bootstrapper) and `EngineContext.cs` (51 lines, static singleton accessor via `Singleton<IEngine>.Instance`).

**Nop.Data** — a thin persistence layer:
- `IRepository<TEntity>` interface and its single concrete implementation `EntityRepository<TEntity>` (528 lines).
- `INopDataProvider` interface (341 lines) with three providers: `MsSqlNopDataProvider`, `MySqlNopDataProvider`, `PostgreSqlDataProvider` — all using **Linq2DB** (not Entity Framework), all extending a common `BaseDataProvider`.
...
**Nop.Services** — the business-logic layer, containing 37 subdirectories that mirror and extend the domain:
...
- 114 `CacheEventConsumer` classes for cache invalidation.
- Plugin management (`IPlugin`, `BasePlugin`, `PluginManager<T>`, and specialised plugin managers).
- Scheduling (`IScheduleTask`, `ScheduleTaskRunner`).
- 4 custom middleware classes: `KeepAliveMiddleware` (`Nop.Services/Common/`), `ThemesMiddleware` (`Nop.Services/Themes/`), `InstallUrlMiddleware` (`Nop.Services/Installation/`), `AuthenticationMiddleware` (`Nop.Services/Authentication/`).
- The custom `ILogger` interface (database-backed, 115 lines) — this is **not** `Microsoft.Extensions.Logging.ILogger`.

**Nop.Web.Framework** — shared web infrastructure:
- 11 `INopStartup` implementations (e.g., `NopMvcStartup`, `ErrorHandlerStartup`, `AuthenticationStartup`, `NopRoutingStartup`, `NopCommonStartup`, `NopStaticFilesStartup`, `NopWebMarkupMinStartup`, `NopProxyStartup`, `AuthorizationStartup`, `NopEndpoints`).
- Base controllers: `BaseController`, `BasePluginController`, `BasePaymentController`.
- The central DI registration file `NopStartup.cs` (334 lines, `Order = 2000`) — this is where **every** service interface is manually mapped to its implementation (line 73–312), and where all `IConsumer<T>` implementations are auto-discovered and registered (line 305–312).

**Nop.Web** — the executable host:
- `Program.cs` (51 lines) bootstraps the application.
- `Areas/Admin/` — a full MVC Area for the backoffice, with its own `BaseAdminController`.
- `Factories/` — 42 ViewModel factory files (`IProductModelFactory`, `ICatalogModelFactory`, etc.) implementing the **Thin Controller** pattern.
...
3. **Service Locator**: `EngineContext.Current.Resolve<T>()` is used where constructor injection is impractical (e.g., static extension methods, middleware configuration). This is visible throughout `ApplicationBuilderExtensions.cs` (25 usages of `EngineContext.Current.Resolve`).
...
1. **Resolves all consumers** dynamically from the DI container (`EventPublisher.cs:23`):
   ```csharp
   var consumers = EngineContext.Current.ResolveAll<IConsumer<TEvent>>().ToList();
   ```
2. **Iterates and invokes** each consumer's `HandleEventAsync()` **sequentially** (not in parallel) via a `foreach` loop (line 25).
3. **Handles errors gracefully**: if a consumer throws, the exception is logged via `ILogger` (with a nested try-catch to prevent logging loops, lines 35–49). Other consumers continue executing.
4. **Supports chain interruption**: if the event implements `IStopProcessingEvent` and its `StopProcessing` property is `true` (line 32), the loop breaks early — no further consumers are called.
...
- Application lifecycle events like `AppStartedEvent` (`ApplicationBuilderExtensions.cs`).

### 2.5 Event Consumers in Practice

The most prominent use of the event system is **cache invalidation**. There are **114 `CacheEventConsumer` classes** across `Nop.Services/`, each subscribing to entity lifecycle events to clear the relevant cache entries:
...
The `IEventPublisher` / `IConsumer<T>` system is the **single best insertion point for observability** on write operations. Because every entity write operation automatically publishes an event, an instrumentation plugin can:
...
Every entity, service, and infrastructure class is declared as `partial class` — there are approximately 970 `partial class` or `partial interface` declarations across the Libraries. For example: `public partial class ProductService : IProductService` (`ProductService.cs:26`). This allows developers to extend any class with additional methods in a separate file without modifying the original source — useful for adding instrumentation methods or hooks.

#### 3.1.4 The `virtual` Keyword on Service Methods

Service methods are consistently marked as `virtual`, enabling subclass-based instrumentation. For instance, `ProductService.SearchProductsAsync()` (line 807) and `GetProductByIdAsync()` (line 552) are both `virtual`. A plugin can inherit from `ProductService`, override key methods to add tracing, and re-register the binding in an `INopStartup`.
...
This can be done without touching `Program.cs`. An `INopStartup` with a higher `Order` value (e.g., 2001) runs after the default `NopStartup` (Order 2000), allowing it to replace DI registrations.

#### 3.1.6 The ASP.NET Core Middleware Pipeline

The HTTP request pipeline is built through `ApplicationBuilderExtensions.cs` with standard ASP.NET Core middleware. Adding a tracing or metrics middleware (e.g., OpenTelemetry ASP.NET Core instrumentation) is a matter of inserting appropriate calls at the right point in the pipeline. The `INopStartup.Configure()` method provides the extension point to do this.
...
- **`ICustomerActivityService` / `CustomerActivityService`** (`Nop.Services/Logging/`): An audit-trail system that tracks user actions. Calls like `InsertActivityAsync("EditProduct", "Edited product: {Name}", product)` are scattered throughout admin controllers (150 usages across 38 admin controller files). Each `ActivityLog` record contains: a `SystemKeyword` (e.g., `"AddNewProduct"`, `"EditOrder"`), a comment string, the customer ID, IP address, entity name, entity ID, and creation timestamp.

These two systems provide a foundation: they demonstrate that the codebase already has "observability awareness" at the application level. However, they both write exclusively to the database, have no structured context or correlation IDs, and are invisible to external monitoring tools.

### 3.2 Where Observability Is Hard

#### 3.2.1 The Custom `ILogger` Is Not `Microsoft.Extensions.Logging.ILogger`

This is the **most significant observability obstacle**. nopCommerce defines its own `Nop.Services.Logging.ILogger` (`ILogger.cs`, 115 lines) — a database-backed logger with methods like `InsertLogAsync(LogLevel, shortMessage, fullMessage, customer)`. It is **not** the standard `Microsoft.Extensions.Logging.ILogger`.
...
- All internal error logging (e.g., `EventPublisher` catching consumer exceptions, `ApplicationBuilderExtensions` logging errors) goes through this custom `ILogger`, bypassing the standard .NET pipeline entirely.
...
- `ApplicationBuilderExtensions.cs` — 25 usages of `EngineContext.Current.Resolve<>()`
...
A search for `OpenTelemetry`, `Telemetry`, `System.Diagnostics.Activity`, `ActivitySource`, and `DiagnosticSource` across the entire original codebase returned **zero results**. There is:
...
- **Write-only coverage**: The event system only fires on entity mutations (insert, update, delete). Read operations like product searches and product views — which form the most common user flow — are not covered by any events.
...
### 4.1 Change 1: Add OpenTelemetry Middleware via INopStartup

**What to change**: Create a new `INopStartup` implementation that:
...
### 4.2 Change 2: Bridge the Custom Logger to `Microsoft.Extensions.Logging`
...
**Is it worth it**: **Absolutely**. This is a high-impact, low-effort change. Without it, all application logs are trapped in a database table with no structured context.
...
Then replace the DI registration in `NopDbStartup.cs` to register `InstrumentedRepository<T>` as the implementation of `IRepository<T>`.
...
**How invasive**: **Low to medium**. The DI registration for `IRepository<T>` is an open generic binding in `Nop.Data/NopDbStartup.cs:64`: `services.AddScoped(typeof(IRepository<>), typeof(EntityRepository<>))`. The decorator must be inserted by overriding this registration. This is a single-file, two-line change.

**Is it worth it**: **Yes**. Database access is typically the most critical layer to instrument. Without this, all SQL operations are invisible to the tracing pipeline. The generic nature of `IRepository<T>` makes a single decorator cover all entities.
...
- (Option B) Use subclass overrides (leveraging `virtual` methods) or DI interceptors to add timing around service method calls.
...
- Option B: **Low**. Subclass overrides or DI interceptors can be configured in a custom `INopStartup` without modifying existing classes.

**Is it worth it**: **Conditionally**. Option B (subclass overrides) is worth it for specific high-value flows. Option A (before events) is a larger change that might not justify the effort if the repository decorator (Change 3) already provides data-layer timing.
...
| **3** | `IRepository<T>` decorator for DB tracing | Low-Medium | High (data layer visibility) | 1 file (DI registration) |
| **4** | Service subclassing for timing (virtual overrides) | Low | Medium (service timing) | No |
...
The first four changes can be implemented with **minimal or zero modifications to the core nopCommerce source code**. This is a testament to the platform's extensible architecture — the same patterns that enable third-party payment gateways and shipping providers also enable clean observability instrumentation.
